...
int main( int argc, char** argv ) try {
    ...
    if ( vm.count( "config" ) ) {
        try {
            ...
            // HACK for testing
            fs::path envPath = configPath.parent_path() / ( chainParams.sChain.name + ".txt" );
            if ( fs::exists( envPath ) ) {
                cerr << "Reading env from " << envPath << endl;
                ifstream ifs( envPath.c_str() );
                std::string good_block_str, nodes_ids_str;
                std::getline( ifs, good_block_str );
                std::getline( ifs, nodes_ids_str );
                setenv(
                    "SKALED_TEST_LAST_GOOD_BLOCK_FOR_AMSTERDAM_FIX", good_block_str.c_str(), 0 );
                setenv( "SKALED_TEST_GOOD_NODES_IDS_FOR_AMSTERDAM_FIX", nodes_ids_str.c_str(), 0 );
            }
